...
from .reset_utils import sample_joint_reset_positions

if TYPE_CHECKING:
    from Gurukul.tasks.direct.brainco_revo.brainco_hand_reorient_env_cfg import BrainCoHandEnvCfg
...
def _collect_collision_prims(stage: Usd.Stage, link_prim_path: str) -> list[Sdf.Path]:
    """Collect all collision prims under a link. Falls back to link prim if none found."""
...
def _get_filtered_pairs_rel(prim: Usd.Prim) -> Usd.Relationship:
    """Return a relationship to author filteredPairs (compatible across USD variants)."""
...
def _build_link_pairs(
    ...
    """Build unique link pairs to be filtered using FilteredPairsAPI."""
    ...
    for group_a, group_b in filtered_group_pairs:
        links_a = group_links.get(group_a, [])
        links_b = group_links.get(group_b, [])
        for a in links_a:
            for b in links_b:
                if a == b:
                    continue
...
def disable_collision_pairs_filtered_direct(
    env: DirectRLEnv,
    ...
    env_ns = getattr(env.scene, "env_ns", "/World/envs")
    for env_id in _resolve_env_ids(env, env_ids):
        env_path = f"{env_ns}/env_{env_id}"
...
class InHandManipulationEnv(DirectRLEnv):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.actuated_joint_names = [self.hand.joint_names[idx] for idx in self.actuated_dof_indices]
        print("action_to_joint_mapping:")
        for action_idx, joint_name in enumerate(self.actuated_joint_names):
            print(f"  action[{action_idx}] -> {joint_name}")
    ...
    def _setup_scene(self):
        ...
        # FilteredPairsAPI (precise filtering): disable collisions between palm and MCP links
        disable_collision_pairs_filtered_direct(
            self,
            env_ids=None,
            asset_root="Robot",
            group_links={
                "paml": ["right_base_link"],
                "mcp": [
    ...
    def _apply_action(self) -> None:
        # Original action-driven control:
    ...
    def _get_observations(self) -> dict:
        ...
        if self.cfg.obs_type == "openai":
            ...
            raise ValueError(f"Unsupported observation type: {self.cfg.obs_type!r}")
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        if len(goal_env_ids) > 0:
            self._reset_target_pose(goal_env_ids, advance_from_current=True)
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        # reset goals
        self._reset_target_pose(env_ids, advance_from_current=False)

        # reset object
        object_default_state = self.object.data.default_root_state.clone()[env_ids]
        # Keep the object reset pose fixed relative to each environment origin.
        ...
        dof_pos = sample_joint_reset_positions(
            self.hand.data.default_joint_pos[env_ids],
    ...
    def _reset_target_pose(self, env_ids, advance_from_current: bool = False):
        # Original random target rotation:
        # rand_floats = sample_uniform(-1.0, 1.0, (len(env_ids), 2), device=self.device)
        # new_rot = randomize_rotation(
        #     rand_floats[:, 0], rand_floats[:, 1], self.x_unit_tensor[env_ids], self.y_unit_tensor[env_ids]
        # )

        yaw_offset = torch.full((len(env_ids),), np.pi / 8.0, dtype=torch.float, device=self.device)
        zero_angles = torch.zeros_like(yaw_offset)

        if advance_from_current:
            # On success, advance the current target by another 22.5 degrees around yaw.
            _, _, current_yaw = euler_xyz_from_quat(self.goal_rot[env_ids])
            new_rot = quat_from_euler_xyz(zero_angles, zero_angles, current_yaw + yaw_offset)
        else:
            # On full env reset, derive the first target from the fixed object in-hand pose:
            # remove roll/pitch and add a single 22.5 degree yaw offset.
            base_object_rot = self.object.data.default_root_state[env_ids, 3:7]
            _, _, base_yaw = euler_xyz_from_quat(base_object_rot)
            new_rot = quat_from_euler_xyz(zero_angles, zero_angles, base_yaw + yaw_offset)
