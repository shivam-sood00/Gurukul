...
USAGE:
    python csv2npz.py --input /path/to/dance1_subject2.csv
...
DEFAULT_G1_DESCRIPTION = EXTENSION_ROOT / "data/Robots/unitree/g1_description"
DEFAULT_URDF = DEFAULT_G1_DESCRIPTION / "urdf/g1_29dof_rev_1_0.urdf"
...
def main():
    parser = argparse.ArgumentParser(description="Convert a retargeted G1 CSV clip to the local AMP NPZ format.")
    parser.add_argument("--input", type=Path, required=True, help="Retargeted G1 CSV file.")
    parser.add_argument("--output", type=Path, help="Output NPZ. Defaults to <input-stem>_30.npz beside the CSV.")
    parser.add_argument("--urdf", type=Path, default=DEFAULT_URDF, help="G1 URDF used for forward kinematics.")
    parser.add_argument(
        "--mesh-dir",
        type=Path,
        default=DEFAULT_G1_DESCRIPTION,
        help="Root directory containing the G1 meshes referenced by the URDF.",
    )
    args = parser.parse_args()

    # 1. Read CSV data
    csv_file = args.input.expanduser().resolve()
    out_filename = (
        args.output.expanduser().resolve()
        if args.output
        else csv_file.with_name(f"{csv_file.stem}_30.npz")
    )
