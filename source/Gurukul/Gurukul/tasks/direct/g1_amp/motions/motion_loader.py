...
class MotionLoader:
    ...
    def __init__(self, motion_file: str, device: torch.device) -> None:
        ...
        Raises:
            FileNotFoundError: If the specified motion file doesn't exist.
        """
        ...
        if self.num_frames < 2:
            raise ValueError(f"Motion must contain at least two frames, got {self.num_frames}.")
    ...
    def _slerp(
        ...
        *,
        ...
        cos_half_theta = torch.abs(cos_half_theta).clamp(max=1.0).unsqueeze(-1)

        half_theta = torch.acos(cos_half_theta)
        sin_half_theta = torch.sqrt(torch.clamp(1.0 - cos_half_theta * cos_half_theta, min=0.0))
        safe_sin_half_theta = sin_half_theta.clamp_min(1.0e-8)

        ratio_a = torch.sin((1 - blend) * half_theta) / safe_sin_half_theta
        ratio_b = torch.sin(blend * half_theta) / safe_sin_half_theta
        ...
        new_q = torch.cat([new_q_w, new_q_x, new_q_y, new_q_z], dim=-1)
        lerped_q = (1.0 - blend) * q0 + blend * q1
        lerped_q = lerped_q / torch.linalg.vector_norm(lerped_q, dim=-1, keepdim=True).clamp_min(1.0e-8)
        return torch.where(sin_half_theta < 1.0e-4, lerped_q, new_q)
    ...
    def _compute_frame_blend(self, times: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        """
        clipped_times = np.clip(np.asarray(times, dtype=np.float64), 0.0, self.duration)
        frame = clipped_times / self.dt
        index_0 = np.floor(frame).astype(int)
        index_1 = np.minimum(index_0 + 1, self.num_frames - 1)
        blend = (frame - index_0).round(decimals=7)
    ...
    def sample_times(self, num_samples: int, duration: float | None = None) -> np.ndarray:
        ...
        Raises:
            ValueError: If the specified duration is outside the motion duration.
        ...
        if not 0.0 <= duration <= self.duration:
            raise ValueError(f"Sample duration must be within [0, {self.duration}], got {duration}.")
        return duration * np.random.uniform(low=0.0, high=1.0, size=num_samples)

    def history_times(self, current_times: np.ndarray, history_length: int, step_dt: float) -> np.ndarray:
        """Return clipped consecutive policy-step timestamps, newest first."""
        if history_length <= 0:
            raise ValueError(f"History length must be positive, got {history_length}.")
        if step_dt <= 0.0:
            raise ValueError(f"History step must be positive, got {step_dt}.")
        offsets = float(step_dt) * np.arange(history_length, dtype=np.float64)
        times = np.asarray(current_times, dtype=np.float64).reshape(-1, 1) - offsets
        return np.clip(times, 0.0, self.duration)
    ...
    def get_dof_index(self, dof_names: list[str]) -> list[int]:
        ...
        Raises:
            ValueError: If a specified DOF name doesn't exist.
        ...
        for name in dof_names:
            if name not in self._dof_names:
                raise ValueError(f"The specified DOF name ({name}) doesn't exist: {self._dof_names}")
    ...
    def get_body_index(self, body_names: list[str]) -> list[int]:
        ...
        Raises:
            ValueError: If a specified body name doesn't exist.
        ...
        for name in body_names:
            if name not in self._body_names:
                raise ValueError(f"The specified body name ({name}) doesn't exist: {self._body_names}")
    ...
    def resample(self, target_dt: float, kind: str = "linear"):
        ...
        if not 0.0 < target_dt <= self.duration:
            raise ValueError(f"target_dt must be within (0, {self.duration}], got {target_dt}.")
        if kind not in {"linear", "cubic"}:
            raise ValueError(f"Unsupported interpolation kind '{kind}'. Expected 'linear' or 'cubic'.")
        ...
        # Keep the timestamp spacing consistent with ``self.dt``. Using linspace here
        # silently changes the actual spacing when the duration is not divisible by
        # target_dt, while still reporting target_dt to downstream samplers.
        target_times = np.arange(target_num_frames) * target_dt
