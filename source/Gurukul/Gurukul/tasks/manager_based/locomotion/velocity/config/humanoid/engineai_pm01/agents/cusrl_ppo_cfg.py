...
@dataclass
class EngineAiPm01RoughTrainerCfg(TrainerCfg):
    max_iterations = 3000
    save_interval = 100
    experiment_name = "engineai_pm01_official_urdf_rough"
...
@dataclass
class EngineAiPm01FlatTrainerCfg(EngineAiPm01RoughTrainerCfg):
    """Align flat network width with engineai_amp ``PM01FlatPPORunnerCfg`` ([128, 128, 128])."""

    max_iterations = 1500
    experiment_name = "engineai_pm01_official_urdf_flat"
    agent_factory = cusrl.ActorCritic.Factory(
        num_steps_per_update=24,
        actor_factory=cusrl.Actor.Factory(
            backbone_factory=cusrl.Mlp.Factory(
                hidden_dims=[128, 128, 128], activation_fn="ELU", ends_with_activation=True
            ),
            distribution_factory=cusrl.NormalDist.Factory(),
        ),
        critic_factory=cusrl.Value.Factory(
