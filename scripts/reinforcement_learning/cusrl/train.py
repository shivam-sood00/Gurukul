...
def _checkpoint_iteration(checkpoint_path: str | None) -> int:
    """Best-effort extraction of the iteration from model_<iter>.pt checkpoint names."""
...
def _configure_iteration_scaled_curricula(env_cfg, agent_cfg, resume_iteration: int = 0) -> None:
    """Inject resolved CusRL iteration counts into curricula that use fractional iteration bins."""
    ...
    agent_factory = getattr(agent_cfg, "agent_factory", None)
    steps_per_iteration = int(getattr(agent_factory, "num_steps_per_update", 0) or 0)
    ...
    for curriculum_name, term_cfg in vars(curriculum_cfg).items():
        ...
        if not isinstance(params, dict):
            continue
        if "stage_iteration_bins" not in params and "arm_difficulty_iteration_bins" not in params:
            continue
        ...
        print(
            f"[INFO] {curriculum_name}: scaled curriculum bins with "
            f"max_iterations={total_iterations}, resume_iteration={resume_iteration}, "
            f"num_steps_per_update={steps_per_iteration}, "
            f"total_env_steps={total_iterations * steps_per_iteration}, "
            f"stage_bins={params.get('stage_iteration_bins')}, "
            f"arm_difficulty_bins={params.get('arm_difficulty_iteration_bins')}."
        )
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: TrainerCfg):
    ...
    _configure_iteration_scaled_curricula(env_cfg, agent_cfg, resume_iteration=_checkpoint_iteration(args_cli.checkpoint))
