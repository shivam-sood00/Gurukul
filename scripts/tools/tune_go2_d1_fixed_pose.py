...
"""Interactively tune the fixed Go2 + D1 arm start pose.

Type commands in the terminal while the Isaac Sim viewer is open:

* ``2=-75`` sets ``arm_2_joint`` to -75 degrees.
* ``j2 -75`` and ``arm_2 -75`` are equivalent.
* ``pose 0 -80 10 0 0 0`` sets the six arm angles in degrees.
* ``select 3`` selects ``arm_3_joint`` for incremental edits.
...
parser = argparse.ArgumentParser(description="Tune the fixed Go2 + D1 gripper start pose.")
...
parser.add_argument(
    ...
    nargs="+",
    metavar="VALUE",
    help="Initial pose: 6 arm angles in degrees, optionally followed by 2 gripper positions in meters.",
)
...
from Gurukul.assets.unitree import UNITREE_GO2_D1_ARM_CFG
...
def _make_scene_cfg() -> InteractiveSceneCfg:
    robot_cfg = UNITREE_GO2_D1_ARM_CFG.replace(prim_path="/World/Robot")
    ...
    if args_cli.pose is not None:
        if len(args_cli.pose) not in (6, 8):
            raise ValueError("--pose expects 6 arm angles in degrees, optionally followed by 2 gripper positions.")
...
def _print_pose(pose_rad: dict[str, float]) -> None:
    ...
    for name in GRIPPER_JOINTS:
        print(f"  {name}: {pose_rad[name]:8.4f} m")
    print("\nPython radians:")
    for name in REVOLUTE_ARM_JOINTS:
...
def _print_help() -> None:
    print(
        """
Commands:
  2=-75                 set arm_2_joint to -75 deg
  j2 -75                same as above
  arm_2 -75             same as above
  arm2=-75              same as above
  g1 0.01               set arm_7_1_joint to 0.01 m
  pose 0 -80 10 0 0 0
                         set arm_1..arm_6 in degrees
  pose 0 -80 10 0 0 0 0.0 0.0
                         set arm angles in degrees, gripper in meters
  select 3              select arm_3_joint
...
def _parse_joint_token(token: str) -> str | None:
    ...
    if token in {"1", "2", "3", "4", "5", "6"}:
        return f"arm_{token}_joint"
    if token in {"7_1", "g1"}:
        return "arm_7_1_joint"
    if token in {"7_2", "g2"}:
        return "arm_7_2_joint"
    return None
...
def _format_joint_value(pose_rad: dict[str, float], joint_name: str) -> str:
    if joint_name in GRIPPER_JOINTS:
        return f"{pose_rad[joint_name]:.4f} m"
...
def _handle_command(line: str, pose_rad: dict[str, float], selected: str) -> tuple[str, bool]:
    ...
    if cmd == "pose":
        if len(parts) not in (len(REVOLUTE_ARM_JOINTS) + 1, len(ARM_JOINTS) + 1):
            print("[WARN] pose expects 6 arm angles in degrees, optionally followed by 2 gripper positions in meters.")
            return selected, False
        try:
...
def main() -> None:
    ...
    selected = "arm_2_joint"

    print("[INFO] Go2+D1 fixed-pose tuner")
    print("[INFO] Commands: 2=-75 | pose 0 -80 10 0 0 0 | + | - | show | help | quit")
    _print_pose(pose_rad)
