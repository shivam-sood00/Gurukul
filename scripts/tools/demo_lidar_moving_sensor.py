"""Visual demo: move and rotate RealisticLidar around static obstacles."""
...
parser = argparse.ArgumentParser(description="Demo RealisticLidar frame transforms with a moving sensor.")
...
def main() -> None:
    ...
    translate_op, orient_op = define_xform("/World/Lidar", (0.0, 0.0, 0.9))
    add_light()
    ...
    while _should_continue(step, sim):
        ...
        radius = 0.85
        translate_op.Set(Gf.Vec3d(0.5 + radius * math.cos(0.6 * t), radius * math.sin(0.6 * t), 0.9))
        half_yaw = 0.5 * 0.8 * t
        orient_op.Set(Gf.Quatf(math.cos(half_yaw), 0.0, 0.0, math.sin(half_yaw)))
        ...
        if step == 0 or (args_cli.print_every > 0 and step % args_cli.print_every == 0):
            print_lidar_stats("moving", lidar, step)
            valid = lidar.get_valid_mask()
            if torch.any(valid):
                converted = lidar.sensor_to_world(lidar.get_pointcloud(frame="sensor"))
                error = torch.linalg.norm(converted - lidar.get_pointcloud(frame="world"), dim=-1)
                print(f"[INFO] moving step={step}: sensor_to_world max_error={float(error[valid].max()):.6f} m")
