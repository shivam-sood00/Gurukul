...
class RLController(object):
    ...
    # Export this from `unitree_b2_z1_arm_moving_flat_distill` or override with GURUKUL_POLICY_PATH.
    actor_path = 'policies/B2Z1_leg_control_22May.onnx'
    ...
    def __init__(self):
        ...
        self.dof_pos = np.zeros((1, self.num_motors), dtype=np.float32)
        self.dof_vel = np.zeros((1, self.num_motors), dtype=np.float32)
        ...
        self.commands_scaled = np.zeros((1, 5), dtype=np.float32)
    ...
    def get_obs(self):
        ...
        dof_info = np.array(list(map(lambda x: x, self.low_state.motor_state))[:self.num_motors])
        for i in range(self.num_motors):
            self.dof_pos[0, i] = dof_info[i].q - self.default_joint_angles[i]
            self.dof_vel[0, i] = dof_info[i].dq
        ...
        self.commands_scaled[0, 3] = 0.0
        self.commands_scaled[0, 4] = 0.0
    ...
    def run(self):
        ...
        for i, a in enumerate(self.mapping):
            ...
            self.cmd.motor_cmd[i].tau = 0.0
        for i in range(self.num_actions, self.num_motors):
            self.cmd.motor_cmd[i].q = self.default_joint_angles[i]
            self.cmd.motor_cmd[i].kp = float(self.kps[i])
            self.cmd.motor_cmd[i].dq = 0.0
            self.cmd.motor_cmd[i].kd = float(self.kds[i])
            self.cmd.motor_cmd[i].tau = 0.0
    ...
    def stand_up(self):
        ...
        stand_down_joint_pos = np.array([self.low_state.motor_state[i].q for i in range(self.num_motors)], dtype=float)
    ...
    def load_policy(self):
        if not Path(self.actor_path).is_file():
            raise FileNotFoundError(
                f"B2+Z1 distilled policy file not found: {self.actor_path}. "
                "Set GURUKUL_POLICY_PATH to the exported unitree_b2_z1_arm_moving_flat_distill ONNX."
            )
...
if __name__ == '__main__':
    print("WARNING: B2+Z1 hardware runner. Ensure the robot is supported and the area is clear.")
