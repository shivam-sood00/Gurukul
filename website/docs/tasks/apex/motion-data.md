...
The public repository includes the project-authored Go2+D1 demonstrations and the APEX Go2 subset listed below.
Other third-party motion collections remain external.
...
```text
source/Gurukul/Gurukul/tasks/manager_based/go2_apex/config/go2/motion/npz/animal_mocap/go2_retarget_canter_2ms.npz
```
...
Git except for the Go2+D1 branches and the bundled APEX Go2 subset. NPZ files use Git LFS; run `git lfs pull` after
cloning to fetch the actual motion arrays.

The APEX subset contains seven original CSVs under `animal_mocap/`, `gait_switch/`, and `walk_these_ways/`, plus ten
validated NPZs in the matching `npz/` subdirectories: canter, trot, pace, jump, hop-turn, pronking, and four segments
of the combined gait recording. Raw CSVs are preserved. Converted canter/trot/pace/jump/pronking clips select
`[0, 1000)` to exclude their corrupt reset frame; hop-turn keeps all 181 frames. The combined recording is split
into `[0, 400)`, `[400, 800)`, `[800, 1200)`, and `[1200, 1611)` at root-position discontinuities. Its NPZ names end in
`_frames_START_STOP.npz`; no gait labels are inferred from the filename. All conversions use 50 Hz.

`imitation_data/APEX_GO2_IMPORT.json` records the source repository, checksums, frame selections, and coordinate
conventions.
...
Go2+D1 directories listed in `motion/GO2_D1_DATA_PROVENANCE.md`; those files are included. The bundled APEX Go2 subset
is also project-authored and recorded in its import manifest. For every other group,
...
python source/Gurukul/Gurukul/tasks/manager_based/go2_apex/config/go2/motion/apex_csv_to_motion_npz.py \
  ...
  --output source/Gurukul/Gurukul/tasks/manager_based/go2_apex/config/go2/motion/npz/animal_mocap/go2_retarget_canter_2ms.npz \
  --fps 50 --frame-range 0 1000 \
  --velocity-frame body --feet-frame yaw \
  --csv-leg-order FL,FR,RL,RR
```

`--frame-range START END` selects numeric frames with an exclusive end before computing velocities. The canter
selection above keeps frames `0–999`; frame `1000` in the source recording has impossible foot positions.
The converter rejects malformed rows, nonfinite values, zero quaternions, fewer than two selected frames, and
feet more than `0.9 m` from the Go2 root. It reports the offending frame instead of silently dropping it. Training
also checks loaded NPZ arrays and foot reach, including existing files.

For ordinary Go2 APEX CSVs, `--velocity-frame auto` treats recorded base linear/angular velocities as body-frame
vectors and rotates them into world coordinates. `--feet-frame auto` treats the foot offsets as yaw-frame vectors.
Use explicit `body`, `world`, or `yaw` options when the source uses another convention (`yaw` applies to feet
only). STMR input retains world-frame velocities by default; named Go2+D1 input retains its world-velocity and
body-foot defaults. Foot positions generated by forward kinematics use the full body rotation.

The batch converter accepts the same frame-selection and coordinate options. Reconvert older ordinary Go2 APEX
CSV-derived NPZ files to correct their velocity and foot frames; a reach check alone cannot detect mislabeled
coordinates. New NPZ metadata records the conversion version, selected range, and resolved source frames.
